...
class CWidgetConfig {
	...
	public static function getKnownWidgetTypes(string $context): array {
		...
		foreach (APP::Component()->widgetmanager->getRegisteredWidgets() as $widgetClass) {
			$types += [$widgetClass::UNIQUEID => $widgetClass::getLabel()];
		}
	...
	public static function getJSClasses(): array {
		...
		] + APP::Component()->widgetmanager->getJavascriptClasses();
	}
	...
	private static function getDefaultDimensions(): array {
		...
		] + APP::Component()->widgetmanager->getDefaultDimensions();
	}
	...
	public static function getDefaultRfRate(string $type): int {
		...
		$refresh = APP::Component()->widgetmanager->getDefaultRfRate();

		return array_key_exists($type, $refresh) ? $refresh[$type] : 0;
	}
	...
	public static function getForm(string $type, string $data, ?string $templateid): CWidgetForm {
		$widgets = APP::Component()->widgetmanager->getRegisteredWidgets();

		if (array_key_exists($type, $widgets)) {
			$widget_class = $widgets[$type];

			return new $widget_class($data, $templateid);
		}
